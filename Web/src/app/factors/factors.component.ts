...
export class FactorsComponent implements OnInit {
  ...
  constructor(private router: Router,private alertService: AlertService,private factorsService: FactorsService,) { }

  ngOnInit() {
    AOS.init();
    if(this.router.url === '/factors'){
      document.body.style.backgroundImage = "url('assets/images/factors/factorsbanner2.jpg')"; 
  ...
  SaveActivationDeactivation(){
    ...
    this.factorsService.saveservice(this.loggedInUser,this.loggedInUser).subscribe(data => {
